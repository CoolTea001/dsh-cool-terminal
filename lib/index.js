...
var PtyRegistry = class {
	...
	*/
	...
	forget(session) {
		...
		if (session.idle !== void 0) clearTimeout(session.idle);
		if (session.reap !== void 0) clearTimeout(session.reap);
		session.idle = void 0;
		session.reap = void 0;
	}
	/**
	...
	*/
	...
	attach(session, res) {
		if (session.reap !== void 0) {
			clearTimeout(session.reap);
			session.reap = void 0;
		}
		if (session.idle !== void 0) {
			clearTimeout(session.idle);
			session.idle = void 0;
		}
	...
	async close(id) {
		...
		if (session.exited === null) session.exited = {
			exitCode: null,
			signal: null
		};
		this.forget(session);
	...
	ctx.effect(() => {
		const disposers = [
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.context,
				handler: guard((req, res) => handleContext(ctx, req, res))
			}),
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.open,
				handler: guard((req, res) => handleOpen(registry, req, res))
			}),
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.stream,
				handler: guard((req, res) => handleStream(registry, req, res))
			}),
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.input,
				handler: guard((req, res) => handleInput(registry, req, res))
			}),
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.signal,
				handler: guard((req, res) => handleSignal(registry, req, res))
			}),
			ctx.webServer.register({
				kind: "exact",
				path: ROUTES.close,
				handler: guard((req, res) => handleClose(registry, req, res))
			})
		];
		return () => {
