...
- 每个终端独立保留 shell 状态：`cd`、导出的变量、正在运行的命令，都能跨 tab 切换**以及刷新页面**存活。Host 按终端 id 记录每个 shell，刷新后的页面会重新接回自己原有的那个 shell 并重放最近输出，而不是把它丢在后台另开一个。
- 终端名称按浏览器持久化，每个终端各自保留滚动历史（5000 行）。
- 最近激活的终端同样会被记住：刷新页面后重新打开的就是它（所在分组若已收起会自动展开），而不是跳回第一个分组的第一个终端。
...
- 终端是进程内状态，DSH 重启后不会保留。长时间没有浏览器连接的终端会在 15 分钟后自动关闭。刷新后重新接回时，恢复的是最近约 256 KB 的输出，不保证是完整滚动历史。
